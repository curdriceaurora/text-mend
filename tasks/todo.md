...
- ~~Corpus remains a compact hand-curated list~~ **Done (post-V1.5):** swapped to a 20,000-word
  frequency-ranked list (`src/core/corpus-data.js`, generated by `tools/build-corpus.mjs` from
  hermitdave/FrequencyWords, MIT; `npm run build:corpus`). Real-world hyphenation joins now fire
  (`extension`, `cooperate` are covered). False-positive margin verified: clean corpus tops out
  at 0.12 confidence vs the 0.75 threshold; an all-reversible-words adversarial sentence reaches
  only 0.24 and stays `none`.
  - **Review follow-ups (all addressed):** build now pins the upstream commit + verifies a
    SHA-256 of the download (reproducible/tamper-evident) and applies a profanity/non-English
    denylist; NOTICE.md credits the OpenSubtitles/OPUS upstream and clarifies only a derived
    word list (no subtitle text) is bundled; FP corpus expanded 25 → 104 real sentences
    (still 0 false positives, max 0.16); added `test/corpus.test.js` (size/membership guard,
    gives `COMMON_WORD_COUNT` a consumer); re-added single letters `a`/`i` dropped by the filter.
